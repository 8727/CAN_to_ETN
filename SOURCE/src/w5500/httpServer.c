...
void reg_httpServer_webContent(uint8_t * content_name, uint8_t * content){
  ...
  web_content[total_content_cnt].content_name = malloc(name_len + 0x01);
  strcpy((char *)web_content[total_content_cnt].content_name, (char *)content_name);
...
uint8_t display_reg_webContent_list(void){
  ...
  if(total_content_cnt == 0){
    ...
    for(i = 0; i < total_content_cnt; i++){
      ...
      printf("%ld byte, ", (long)web_content[i].content_len);
...
uint8_t find_userReg_webContent(uint8_t * content_name, uint16_t * content_num, uint32_t * file_len){
  uint16_t i;
  uint8_t ret = false; // '0' means 'File Not Found'
  for(i = 0; i < total_content_cnt; i++){
    if(0x00 == strcmp((char *)content_name, (char *)web_content[i].content_name)){
      *file_len = web_content[i].content_len;
      *content_num = i;
      ret = true; // If the requested content found, ret set to '1' (Found)
      break;
    }
  }
  return ret;
